...
export const POST = requireSuperAdmin(async (request: NextRequest) => {
  try {
    ...
    if (validated.format === 'excel') {
      const excelBuffer = await convertExportDataToExcel(exportData);
 ...
 */
async function convertExportDataToExcel(exportData: any): Promise<Buffer> {
  ...
  if (sections.length === 1) {
    ...
    return await arrayToExcel(exportData[sectionName], {
