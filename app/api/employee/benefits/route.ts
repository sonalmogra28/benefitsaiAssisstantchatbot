...
export const GET = withAuth(USER_ROLES.EMPLOYEE)(
  async (request: NextRequest) => {
    try {
      ...
      const enrollments = await benefitService.getEmployeeEnrollments(userId);
      
      // Get all benefit plans to map enrollments to plan details
      const allPlans = await benefitService.getBenefitPlans(userCompanyId);
      ...
      const healthEnrollment = enrollments.find(e => {
        ...
        return plan?.type === 'medical' || plan?.category === 'health';
      });
      ...
      if (healthEnrollment) {
        ...
        if (plan) {
          healthPlan = {
            ...
            premiumTotal: (plan.monthlyCost || plan.monthlyPremium || 0) * 12,
          };
        }
      }
      ...
      const coverageTypes = enrollments.map((enrollment) => {
        ...
        return {
          ...
          monthlyPremium: plan?.monthlyCost || plan?.monthlyPremium || enrollment.monthlyCost || 0,
      ...
      const healthPlans = allPlans.filter(plan => plan.type === 'medical' || plan.category === 'health');
      ...
      if (healthPlans.length > 1 && healthEnrollment) {
        ...
        if (currentPlan && otherPlans.length > 0) {
          const cheapestPlan = otherPlans.reduce((cheapest, plan) => 
            (plan.monthlyCost || plan.monthlyPremium || 0) < (cheapest.monthlyCost || cheapest.monthlyPremium || 0) ? plan : cheapest
          );
          
          const potentialSavings = ((currentPlan.monthlyCost || currentPlan.monthlyPremium || 0) - (cheapestPlan.monthlyCost || cheapestPlan.monthlyPremium || 0)) * 12;
