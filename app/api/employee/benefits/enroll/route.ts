...
export const POST = withAuth(USER_ROLES.EMPLOYEE)(
  async (request: NextRequest) => {
    try {
      ...
      const companyPlans = await benefitService.getBenefitPlans(userCompanyId);
      ...
      return NextResponse.json(
        {
          ...
          enrollment: {
            id: enrollmentId,
            planName: plan.name,
            coverageType: validated.coverageTier,
      ...
      logError('Error enrolling in benefit:', error);
      return NextResponse.json(
...
export const DELETE = withAuth(USER_ROLES.EMPLOYEE)(
  async (request: NextRequest) => {
    try {
      // Extract user information from headers
      const userId = request.headers.get('x-user-id')!;
      ...
      await benefitService.cancelEnrollment(userId, enrollmentId);

      return NextResponse.json({ success: true });
    } catch (error) {
      logError('Error cancelling enrollment:', error);
      return NextResponse.json(
