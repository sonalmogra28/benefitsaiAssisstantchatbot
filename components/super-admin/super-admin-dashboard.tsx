...
export function SuperAdminDashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      // Using Azure AD B2C auth context
      if (!user) {
        console.error('No authenticated user');
        return;
      }

      // TODO: Get token from Azure AD B2C
      ...
      if (statsResponse.ok) {
        ...
        if (response.success && response.data) {
          ...
          logError('Invalid stats response format:', response);
          setDefaultStats();
        }
      } else {
        logError('Failed to fetch stats:', statsResponse.statusText);
        setDefaultStats();
      }
      ...
      if (activityResponse.ok) {
        ...
        logError('Failed to fetch activity:', activityResponse.statusText);
        setRecentActivity([]);
      }
    } catch (error) {
