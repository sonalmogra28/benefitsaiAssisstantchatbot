...
export async function processDocument(documentId: string): Promise<ProcessedDocument> {
  ...
  try {
    ...
    const fileName = document.fileUrl.split('/').pop() || document.id;
    const fileBuffer = await storageServices.documents.downloadFile(fileName);
...
export async function searchDocuments(
  ...
  try {
    ...
    const chunks = await repositories.documentChunks.query({
      query: 'SELECT * FROM c WHERE c.metadata.companyId = @companyId',
