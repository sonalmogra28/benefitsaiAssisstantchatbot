// Azure Cosmos DB uses native Date objects
type Timestamp = Date;
...
export interface BenefitEnrollment extends DocumentBase {
  userId: string;
  companyId: string;
  benefitPlanId: string;
  enrollmentStatus: 'enrolled' | 'pending' | 'cancelled' | 'not_enrolled';
  enrollmentDate?: Timestamp | Date;
  effectiveDate?: Timestamp | Date;
  coverageAmount?: number;
  beneficiaryInfo?: Record<string, any>;
  dependentInfo?: Record<string, any>[];
  notes?: string;
}

// Export as benefitEnrollments for compatibility
export const benefitEnrollments = {} as any;