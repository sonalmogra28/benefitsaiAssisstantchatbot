...
export class ComplianceManager {
  ...
  async validateDataAccess(
    ...
    try {
      ...
      const err = error as Error;
      logger.error({
        msg: 'Data access validation failed',
        userId,
        tenantId,
        resource,
        action,
  ...
  async encryptSensitiveData(data: string, context: string): Promise<string> {
    ...
    try {
      ...
      const err = error as Error;
      logger.error({ msg: 'Data encryption failed', context, error: err?.message ?? String(err) });
      throw error;
    }
  }
  ...
  async decryptSensitiveData(encryptedData: string, context: string): Promise<string> {
    ...
    try {
      ...
      logger.error('Data decryption failed', { context, error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async applyDataRetention(): Promise<void> {
    ...
    try {
      ...
      logger.error('Data retention policy failed', { error: error instanceof Error ? error.message : String(error) });
    }
  }
