...
class AnalyticsService {
  private async getActiveUsersCount(repositories: any, days: number = 30): Promise<number> {
    try {
      ...
      const users = await repositories.users.query({
        query: 'SELECT * FROM c WHERE c.lastActive >= @cutoffDate',
        parameters: [{ name: 'cutoffDate', value: cutoffDate.toISOString() }]
      });
      
      return users.length;
    } catch (error) {
      logger.error({ data: error }, 'Error getting active users count');
      return 0;
    }
  }
  ...
  private async getTotalMessagesCount(repositories: any): Promise<number> {
    try {
      ...
      logger.error({ data: error }, 'Error getting total messages count');
      return 0;
    }
  }
  ...
  private async getStorageUsed(repositories: any): Promise<number> {
    try {
      const documents = await repositories.documents.query({
        query: 'SELECT c.size FROM c WHERE c.size IS NOT NULL'
      });
      
      return documents.reduce((total: number, doc: any) => total + (doc.size || 0), 0);
    } catch (error) {
      logger.error({ data: error }, 'Error getting storage used');
      return 0;
    }
  }
  ...
  private async getAverageResponseTime(repositories: any): Promise<number> {
    try {
      const messages = await repositories.messages.query({
        query: 'SELECT c.responseTime FROM c WHERE c.responseTime IS NOT NULL AND c.role = "assistant"'
      });
      
      if (messages.length === 0) return 0;
      
      const totalTime = messages.reduce((sum: number, msg: any) => sum + (msg.responseTime || 0), 0);
      return Math.round(totalTime / messages.length);
    } catch (error) {
      logger.error({ data: error }, 'Error getting average response time');
      return 0;
    }
  }
  ...
  private async getSatisfactionRate(repositories: any): Promise<number> {
    try {
      ...
      const totalRating = feedback.reduce((sum: number, fb: any) => sum + (fb.rating || 0), 0);
      return Math.round((totalRating / feedback.length) * 100) / 100;
    } catch (error) {
      logger.error({ data: error }, 'Error getting satisfaction rate');
      return 0;
    }
  }
  ...
  private async getSystemMetrics(): Promise<{
    ...
    try {
      ...
      logger.error({ data: error }, 'Failed to get system metrics');
      return {
        cpuUsage: 0,
        memoryUsage: 0,
        diskUsage: 0,
  ...
  private async getDiskUsage(): Promise<number> {
    try {
      ...
      logger.warn({ data: error }, 'Failed to get disk usage');
      return 0;
    }
  }
  ...
  private async getNetworkLatency(): Promise<number> {
    try {
      ...
      logger.warn({ data: error }, 'Failed to get network latency');
      return 0;
    }
  }
  ...
  async getPlatformAnalytics(): Promise<AnalyticsData> {
    try {
      ...
      logger.error({ data: error }, 'Error fetching platform analytics');
      throw error;
    }
  }
  ...
  async getCompanyAnalytics(companyId: string): Promise<CompanyAnalytics> {
    try {
      ...
      const employeeCount = companyUsers.length;
      ...
      const conversationsCount = companyConversations.length;
  ...
  async getUserActivity(companyId?: string): Promise<UserActivity[]> {
    try {
      ...
      const users = await repositories.users.query({ query: usersQuery, parameters });
      ...
      for (const user of users) {
        ...
        const averageSessionDuration = this.calculateAverageSessionDuration(userConversations);
  ...
  async getChatAnalytics(companyId?: string): Promise<ChatAnalytics> {
    try {
      ...
      const conversations = await repositories.chats.query({ query: conversationsQuery, parameters });
      ...
      for (const conversationId of conversationIds) {
        const messages = await repositories.messages.query({
          ...
          parameters: [{ name: 'chatId', value: conversationId }]
        });
