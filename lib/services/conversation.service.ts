...
export async function getConversationAnalytics(companyId: string) {
  try {
    ...
    const chats = await repositories.chats.query({ query, parameters });
    ...
    for (const chat of chats) {
      ...
      const messages = await repositories.messages.query({ query: messageQuery, parameters: messageParams });
...
export async function getSuggestions(
  ...
  try {
    ...
    const chats = await chatsRepository.query({ query, parameters });
