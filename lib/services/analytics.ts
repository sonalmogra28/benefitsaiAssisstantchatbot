...
class AnalyticsService {
      ...
      if (typeof window !== 'undefined') {
        this.appInsights.trackEvent({
          ...
          properties: {
            ...
            isFallback: String(event.isFallback),
          },
          measurements: {
  ...
  async trackUserInteraction(event: UserInteractionEvent): Promise<void> {
    ...
    try {
      ...
      logger.error({ msg: 'Failed to track user interaction', error, event });
    }
  }
  ...
  async getAnalyticsMetrics(
    ...
    try {
      ...
      logger.error({ error, startDate, endDate, companyId }, 'Failed to get analytics metrics');
      throw error;
    }
  }
  ...
  async getDailyCosts(days: number = 30): Promise<Array<{
    ...
    try {
      ...
      logger.error({ error, days }, 'Failed to get daily costs');
      throw error;
    }
  }
}
