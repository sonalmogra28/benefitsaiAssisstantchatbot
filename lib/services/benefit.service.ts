...
class BenefitService {
  ...
  async getBenefitPlan(planId: string): Promise<BenefitPlan | null> {
    try {
      ...
      if (plan) {
        logger.info({ planId }, 'Benefit plan retrieved successfully');
      } else {
        logger.warn({ planId }, 'Benefit plan not found');
      }
  ...
  async getAllBenefitPlans(): Promise<BenefitPlan[]> {
    try {
      ...
      }, 'All benefit plans retrieved successfully');
      
      return plans;
    } catch (error) {
      logger.error({ data: error }, 'Error getting all benefit plans');
