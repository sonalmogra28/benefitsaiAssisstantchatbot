      ...
      loggerCallback: (level, message, containsPii) => {
        ...
        switch (level) {
          case 0: // LogLevel.Error
            logError('MSAL Error', new Error(message));
            break;
...
export class AzureAuthService {
  ...
  async loginRedirect(): Promise<void> {
    try {
      ...
      logError('Login redirect failed', error);
      throw error;
    }
  }
  ...
  async acquireTokenSilent(scopes: string[] = ['openid', 'profile', 'email']): Promise<AuthenticationResult | null> {
    try {
      ...
      logError('Failed to acquire token silently', error);
      return null;
    }
  }
  ...
  async acquireTokenPopup(scopes: string[] = ['openid', 'profile', 'email']): Promise<AuthenticationResult | null> {
    try {
      ...
      logError('Failed to acquire token with popup', error);
      throw error;
    }
  }
  ...
  async handleRedirectPromise(): Promise<AuthenticationResult | null> {
    try {
      ...
      logError('Failed to handle redirect promise', error);
      return null;
    }
  }
  ...
  async getUserRoles(token: string): Promise<string[]> {
    try {
      ...
      console.log('User roles requested', {
        tokenLength: token.length
      });
  ...
  async hasRole(token: string, role: string): Promise<boolean> {
    try {
      ...
      logError('Failed to check user role', error);
      return false;
    }
  }
  ...
  async getUserProfile(token: string): Promise<{
    ...
    try {
      ...
      logError('Failed to get user profile', { error: error instanceof Error ? error.message : 'Unknown error' });
      return null;
    }
  }
}
