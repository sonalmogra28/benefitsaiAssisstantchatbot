...
export class CostOptimizedCommunicationService {
  ...
  private communicationClient: CommunicationIdentityClient;
  ...
  constructor() {
    ...
    try {
      ...
      this.communicationClient = new CommunicationIdentityClient(this.connectionString);
      
      logger.info({}, 'Azure Communication Services initialized successfully');
    } catch (error) {
      logger.error({ error }, 'Failed to initialize Azure Communication Services');
      this.emailClient = null as any;
      this.smsClient = null as any;
      this.communicationClient = null as any;
    }
  ...
  async sendEmail(to: string, subject: string, htmlContent: string, from?: string): Promise<boolean> {
    try {
      ...
      if (result.status === 'Succeeded') {
        ...
        logger.error({ to, subject, status: result.status }, 'Email sending failed');
        return false;
      }
    } catch (error) {
      logger.error({
  ...
  async sendSMS(to: string, message: string, from?: string): Promise<boolean> {
    try {
      ...
      }, 'Sending SMS via Azure Communication Services');

      const results = await this.smsClient.send(smsMessage);

      // results is SmsSendResult[]; pick the first (you only send to one number)
      const first = Array.isArray(results) ? results[0] : (results as any);

      if (first?.successful) {
        logger.info({ to, messageId: first.messageId }, 'SMS sent successfully');
        return true;
      } else {
        logger.error({ to, results }, 'SMS send failed');
        return false;
      }
    } catch (error) {
      logger.error({
  ...
  async createCommunicationUser(): Promise<CommunicationUserIdentifier | null> {
    try {
      ...
      }, 'Communication user created successfully');

      return user;
    } catch (error) {
      logger.error({
        error: error instanceof Error ? error.message : 'Unknown error'
      }, 'Failed to create communication user');
      return null;
    }
  }
  ...
  async deleteCommunicationUser(userId: string): Promise<boolean> {
    try {
      ...
      logger.info({ userId }, 'Communication user deleted successfully');
      return true;
    } catch (error) {
      logger.error({
        error: error instanceof Error ? error.message : 'Unknown error',
        userId
      }, 'Failed to delete communication user');
      return false;
    }
  }
